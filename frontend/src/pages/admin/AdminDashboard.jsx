  ...
  return (
    <div className="container mt-4">
      ...
      <div className="d-flex gap-2 mb-4">
        <button 
          className={`btn ${activeTab === 'professionals' ? 'btn-primary' : 'btn-outline-primary'}`}
          onClick={() => setActiveTab('professionals')}
        >
          👔 Gestionar Profesionales
        </button>
        <button 
          className={`btn ${activeTab === 'contacts' ? 'btn-primary' : 'btn-outline-primary'}`}
      ...
      {activeTab === 'professionals' ? (
        ...
        <div className="professionals-content fade-in">
            ...
            <div className="row mb-3 align-items-center">
                ...
                <Link to="/admin-panel/profesionales/nuevo" className="btn btn-primary">
                    + Nuevo Profesional
                </Link>
                </div>
            </div>
            ...
            <div className="table-responsive shadow-sm rounded">
                ...
                <tbody>
                    ...
                    filteredPros.map((p) => (
                        ...
                        <td>
                            {p.foto ? (
                            <img src={p.foto} alt="avatar" style={{ width: "45px", height: "45px", borderRadius: "50%", objectFit: "cover", border: "2px solid #dee2e6"}} />
                            ) : (
                            <div style={{ width: "45px", height: "45px", borderRadius: "50%", background: "#ccc", display: "flex", alignItems: "center", justifyContent: "center" }}>📷</div>
                            )}
                        </td>
                        <td className="fw-bold">{p.nombre_completo}</td>
                        <td>
                            {p.tipo_servicio === "psicologia" ? (
                            <span className="badge bg-info text-dark">Psicología</span>
                            ) : (
                            <span className="badge bg-warning text-dark">Metodología</span>
                            )}
                        </td>
                        <td>
                            {p.activo ? <span className="badge bg-success">Activo</span> : <span className="badge bg-secondary">Inactivo</span>}
                        </td>
                        <td>
                            <div className="btn-group" role="group">
                            <Link to={`/admin-panel/profesionales/${p.id}/editar`} className="btn btn-primary btn-sm">✏️ Editar</Link>
                            <button onClick={() => handleToggle(p.id, p.activo)} className={`btn btn-sm ${p.activo ? "btn-outline-warning" : "btn-outline-success"}`}>
                                {p.activo ? "Desactivar" : "Activar"}
                            </button>
                            <button onClick={() => handleDelete(p.id)} className="btn btn-outline-danger btn-sm">🗑️</button>
                            </div>
                        </td>
                        </tr>
                    ))
                    ) : (
                    <tr><td colSpan="5" className="text-center py-4 text-muted">No se encontraron profesionales.</td></tr>
                    )}
                </tbody>
                </table>
            </div>
